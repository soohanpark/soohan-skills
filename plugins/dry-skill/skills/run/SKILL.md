---
name: run
description: Use when the user wants to preview what a skill would do without producing real side effects. Activates explicitly via `/run <skill-name>` (Claude Code full id `/dry-skill:run`) or implicitly when the user wraps a request in a dry-run intent (e.g. "dry run the brainstorming skill on this prompt").
---
...
In Explicit mode, `<skill-name>` is either a bare name (`brainstorming`) or a plugin-qualified name (`superpowers:brainstorming`). On hosts without slash commands, the same two modes apply to whatever text activated the skill. In Claude Code the full slash-command identifier is `/dry-skill:run` if `/run` collides with another plugin.

## Dry-Run Contract

You MUST:

1. Locate the target skill's definition. If the host already surfaced its content, use that. Otherwise search the host's skill roots for the skill's `SKILL.md` and read it — `~/.claude/skills/`, `~/.claude/plugins/`, `~/.codex/skills/`, `~/.gemini/skills/`, `~/.kimi/skills/`, `~/.agents/skills/`, plus project-local `.claude/skills/` and `skills/`.
...
3. **NOT** call any side-effecting tool during the simulation:
   - File-writing tools (`Write`/`Edit`/`apply_patch`/`write_file`, notebook editors)
   - Shell commands that mutate state (anything other than read-only inspection)
   - Skill activation itself (no real activation of the target skill — you are simulating it, not running it)
...
4. You **MAY** call read-only tools when needed for accurate simulation:
   - File read, glob, and grep
   - Shell for read-only inspection (`git status`, `git log`, `ls`, `cat` for files outside the project that the read tool can't reach)
...
2. Match against the **available skills list exposed in the current session** (every host advertises its loaded skills with their descriptions). This is the authoritative source — do **not** crawl the filesystem to discover skills.
