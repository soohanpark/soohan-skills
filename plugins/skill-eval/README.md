...
**격리된 빈 디렉터리에서 돌린다.** baseline·forced 변형은 턴 제한 없이 헤드리스로 도는
실제 세션이다. 작업 워크스페이스에서 돌린 실측에서 baseline이 워크스페이스의 다른 프로젝트
문서를 읽고 엉뚱한 답을 냈고(그 비교쌍의 승패는 의미가 없다), 홈 디렉터리 바깥을 훑었으며,
연결된 회사 MCP 서버를 호출했다.
